...
{
public:
	typedef std::pair<unsigned, unsigned> Vertex;
	...
	Vertex WorldToVertex(const Coordinate& coordinate) const
	{
		assert(coordinate.first >= m_x0);
		assert(coordinate.second >= m_y0);
		return Vertex
			(
			static_cast<unsigned>((coordinate.first-m_x0)/m_size.first),
			static_cast<unsigned>((coordinate.second-m_y0)/m_size.second)
			);
	}

	Coordinate VertexToWorld(const Vertex& v) const
	{
		return Coordinate(m_x0+double(v.first)*m_size.first, m_y0+double(v.second)*m_size.second);
	}

	// convert from world coordinate to the coordinate local to the current tile
	static Coordinate WorldToLocal(const Coordinate& world, const Coordinate& origin)
	{
		return Coordinate(world.first-origin.first, world.second-origin.second);
	}
	...
	{
		return tapRectangularTile<Geometry>::Hit(m_size, local);
	}
	...
	void RotateDirectionCounterClockwise(const Vertex& reference, Direction& direction, Path& path) const
	{
