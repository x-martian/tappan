// the source code in this file has been modified by Xiaofeng Zhao
...
namespace lemon {
  ...
  bool diag (const std::string& message) {
    output_ << "# " << message << "\n";
	return true;
  }
  ...
  bool is (const T1& this_one,
           ...
           const std::string& test_name)
  {
    delta *= that_one>0.0 ? that_one : -that_one;
    bool passed = (this_one > that_one-delta && this_one < that_one+delta);
